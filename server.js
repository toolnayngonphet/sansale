...
const cors = require('cors'); // Hỗ trợ Frontend gọi API mượt mà không lo bị chặn
const app = express();
...
// KHO LƯU TRỮ BỘ COOKIE ADMIN (Cập nhật tươi mới liên tục qua API từ xa)
let ADMIN_COOKIES = {
...
// API ĐỂ ADMIN CẬP NHẬT COOKIE TỪ XA
...
app.post('/api/product-info', async (req, res) => {
    try {
        const { url } = req.body;
        if (!url) return res.status(400).json({ success: false, message: 'Thiếu URL sản phẩm' });
...
app.post('/api/split-link', async (req, res) => {
    try {
        ...
        // PHỤC HỒI CHUẨN: Mặc định ban đầu lấy luôn link khách nhập làm Bước 1 để tránh lỗi treo mạng
        let step1VoucherLink = url; 
        let step2AffiliateLink = "";

        // ==================== BƯỚC 1: LẤY VOUCHER TỪ SALESOC (HẠ TIMEOUT XUỐNG 3 GIÂY) ====================
        try {
            const salesocResponse = await axios.post('https://salesoc.vn/api/convert-with-shelf', {
                ...
                timeout: 3000 // GIẢM XUỐNG 3 GIÂY: Nếu IP Railway bị block, ngắt kết nối ngay lập tức để giao diện không bị treo
            });
            ...
            if (salesocResponse && salesocResponse.data && salesocResponse.data.success === true) {
                ...
                if (data.affipadShortUrl && data.affipadShortUrl.trim() !== "") {
                    ...
                    step1VoucherLink = data.shortFacebookAffiliateUrl || data.facebookAffiliateUrl || data.affiliateUrl || url;
                }
            }
        } catch (apiErr) {
            // Khi bị Cloudflare block IP, dòng log này sẽ hiện nhưng step1VoucherLink vẫn giữ nguyên link của khách để chạy bình thường
            console.log('⚠️ Trạng thái Salesoc (Bị block IP hoặc nghẽn):', apiErr.message);
            if (url.includes('s.shopee.vn') || url.includes('shp.ee')) {
                step1VoucherLink = url;
            }
        }

        // ==================== BƯỚC 2: PHÁT TRIỂN CHUYỂN LINK BẰNG COOKIE ADMIN (HẠ TIMEOUT XUỐNG 3 GIÂY) ====================
        try {
            ...
            const shopeeResponse = await axios.post(shopeeGqlUrl, gqlPayload, {
                ...
                timeout: 3000 // GIẢM XUỐNG 3 GIÂY: Chống treo luồng Core khi bị Shopee chặn IP
            });
            ...
            if (shopeeResponse && shopeeResponse.data && shopeeResponse.data.data) {
                ...
                if (dataList.length > 0 && dataList[0].shortLink) {
                    ...
                    step2AffiliateLink = dataList[0].shortLink; 
                }
            }
        } catch (shopeeErr) {
            console.log("⚠️ Lỗi gọi API Shopee trực tiếp (Hết hạn hoặc do IP Cloud bị quét):", shopeeErr.message);
        }
